...
for ARG in $@ ; do
  case "$ARG" in
    ...
    "-d"|"--daemon") UPDATE_DAEMON=1;;
    *) ARGS+=("$ARG");;
  esac
done

if [ -z ${ARGS} ]; then
  echo "usage: newversion [-d|--daemon] [-c|--configs] <new version number> [<new feed>]"
...
if [[ ${UPDATE_DAEMON} -ne 0 ]]; then
  MK_DAEMON="${P44B_SRC_DIR}/../../${DAEMON}/Makefile"
  if [ -f "${MK_DAEMON}" ]; then
    MAKEFILES+=("${MK_DAEMON}")
  else
    warn "- NOT FOUND: ${MK_DAEMON}"
  fi
fi
